<template>
      ...
      <md-card>
          ...
          <div class="md-layout" v-if="capability">
            <div class="md-layout-item md-xsmall-size-100 btn-area">
              <md-button class="error md-sm" @click="dialogDeleteConfirm = true"
                     :disabled="!$store.state.auth.permissions.vessels.includes('U')">Remove
                <md-tooltip>Remove SMFF Service</md-tooltip>
              </md-button>
              <md-button class="primary md-sm" @click="saveChanges"
                    :disabled="!dataChanged || !$store.state.auth.permissions.vessels.includes('U')">
        ...
        <md-card-content>
          <v-alert v-if="alert" :type="alert.type" value="true">{{alert.message}}
            <div v-for="(error, index) in alert.errors" :key="index">
              {{error[0]}}
            </div>
          </v-alert>
          <div class="md-layout" v-if="capability">
          ...
          <div v-else align="center">
            ...
            <md-button class="primary" @click="createSMFF"
                  :disabled="!$store.state.auth.permissions.vessels.includes('U')">Create
            </md-button>
          </div>
        </md-card-content>
  ...
  <!-- <v-card flat>
    <v-card-title>
      <span class="headline">SMFF Services - TABLE 155.4030(b)</span>
      <v-spacer></v-spacer>
      <v-divider vertical v-show="capability"></v-divider>
      <v-btn v-if="capability" :class="!dataChanged ? 'primary' : 'error'" dark text @click="saveChanges"
             :disabled="!dataChanged || !$store.state.auth.permissions.vessels.includes('U')">
    ...
    <v-card-text>
      ...
      <v-layout wrap v-if="capability">
        <v-flex xs12 sm6>
          <v-card>
            <v-card-title class="title grey lighten-2">(1) Salvage</v-card-title>
            <v-divider></v-divider>
            <v-card-text>
              <table>
                <tr>
        ...
        <v-flex xs12 sm6>
          <v-card>
            <v-card-title class="title red lighten-1">(2) Marine firefighting</v-card-title>
            <v-divider></v-divider>
            <v-card-text id="ff">
              <table>
                <tr>
          ...
          <v-card class="mt-2">
            <v-card-title class="title green lighten-1">Primary Service</v-card-title>
            <v-divider></v-divider>
            <v-card-text id="primary_service">
              <v-autocomplete
                  v-model="capability.primary_service"
                  :items="itemsServices"
                  item-text="name"
          ...
          <v-card class="mt-2">
            <v-card-title class="title blue lighten-1">Network Membership</v-card-title>
            <v-divider></v-divider>
            <v-card-text id="network">
              <table>
                <tr>
                  <td>OPA-90 Response Asset</td>
                  <td>
        ...
        <v-flex xs12>
          <v-card>
            <v-card-title class="title grey lighten-2">* Additional Capabilities and Details</v-card-title>
            <v-divider></v-divider>
            <v-card-text>
              <table>
                <tr>
        ...
        <v-flex xs12>
          <v-card>
            <v-card-title class="title grey lighten-2">Notes</v-card-title>
            <v-card-text>
              <v-textarea auto-grow v-model="capability.notes"></v-textarea>
            </v-card-text>
          </v-card>
        ...
        <v-flex xs12>
          <v-card>
            <v-card-text>
              <v-btn block text class="error" @click="dialogDeleteConfirm = true"
                     :disabled="!$store.state.auth.permissions.vessels.includes('U')">Remove SMFF Services
              </v-btn>
            </v-card-text>
          </v-card>
        </v-flex>
  ...
  export default {
    ...
    methods: {
      ...
      updateData (type) {
        ...
        axios.post('vessels/' + this.$route.params.id + '/' + type, data)
          .then(res => {
            this.getDataFromApi()
            this.$store.dispatch('notification/message', { message: res.data.message })
          })
