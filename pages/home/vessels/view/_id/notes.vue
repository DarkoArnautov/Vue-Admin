    ...
    <div class="md-layout-item md-xsmall-size-100" style="padding: 0">
      <md-card>
        <md-card-header>
          ...
          <div class="md-layout">
            <div class="md-layout-item md-xsmall-size-100 btn-area">
              <v-dialog
                  v-model="dialogAddNote"
                  width="500">
                <template v-slot:activator="{ on }">
                  <md-button v-on="on" class="primary md-sm" :disabled="!$store.state.auth.permissions.vessels.includes('U')">
                    Add Note
                  </md-button>
                </template>
                <v-card>
                        ...
                        <v-layout wrap>
                          <v-flex xs12>
                            <v-select
                                ...
                                item-value="id"
                                label="Type*"
                                v-model="newNote.note_type" ></v-select>
                          </v-flex>
                          <v-flex xs12>
                  ...
                  <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn text @click="dialogAddNote = false">Cancel</v-btn>
                    <v-btn color="primary" text @click="save">Add Note</v-btn>
                  </v-card-actions>
                </v-card>
              </v-dialog>
            </div>
        ...
        <md-card-content>
          ...
          <v-alert :value="true" color="info" icon="info" outlined v-show="!notes.length">
            No notes for {{ editedItem.name }}.
          </v-alert>
          <v-card v-for="note in notes" :key="note.id" class="mb-2">
            <v-card-title>{{note.user}} @ {{ note.created_at }}
              <v-spacer></v-spacer>
              <v-btn icon @click="deleteNote(note.id)" :disabled="!$store.state.auth.permissions.vessels.includes('U')">
  ...
  export default {
    ...
    methods: {
      getDataFromApi () {
        this.standBy = true
        this.loading = true
        axios.get('vessels/' + this.$route.params.id + '/short')
          .then(res => {
