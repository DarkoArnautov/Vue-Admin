<template>
  <div class="md-layout">
    <div class="md-layout-item md-xsmall-size-100" style="padding: 0">
      <md-card>
        <md-card-header>
          <h4 class="title">Vessel Drawings</h4>
        </md-card-header>
  ...
  export default {
    ...
    data () {
      return {
        ...
        dropzoneOptions: {
          url: baseURL + 'vessels/' + this.$route.params.id + '/files/drawings/upload',
    ...
    methods: {
      ...
      deleteFile () {
        axios.delete('vessels/' + this.$route.params.id + '/files/drawings/' + this.deleteItem.name + '/destroy')
          .then(res => {
            this.getFiles()
