<template>
  <div class="md-layout">
    <div class="md-layout-item md-xsmall-size-100" style="padding: 0">
      <md-card>
        <md-card-header>
          <h4 class="title">Vessel Fireplans</h4>
        </md-card-header>
  ...
  <!-- <v-card>
    <v-card-title>
      <div class="headline">Vessel Fireplans</div>
    </v-card-title>
    <v-card-text>
      <dropzone id="foo" ref="el" :options="dropzoneOptions" @vdropzone-success="fileUploaded"
                :destroyDropzone="true" v-show="$store.state.auth.permissions.vessels.includes('U')"></dropzone>
  ...
  export default {
    ...
    data () {
      return {
        ...
        dropzoneOptions: {
          url: baseURL + 'vessels/' + this.$route.params.id + '/files/fireplans/upload',
    ...
    methods: {
      ...
      deleteFile () {
        axios.delete('vessels/' + this.$route.params.id + '/files/fireplans/' + this.deleteItem.name + '/destroy')
          .then(res => {
            this.getFiles()
