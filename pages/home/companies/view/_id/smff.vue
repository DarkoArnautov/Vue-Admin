    ...
    <div class="md-layout-item md-xsmall-size-100" style="padding: 0">
        ...
        <md-card-header>
          ...
          <div class="md-layout" v-if="capability">
            <div class="md-layout-item md-xsmall-size-100 btn-area">
              ...
              <md-button v-if="capability" class="primary md-sm" dark text @click="saveChanges"
                    :disabled="!dataChanged || !$store.state.auth.permissions.companies.includes('U')">
        ...
        <md-card-content>
                    ...
                    <md-tab md-label="Primary Service">
                      <v-autocomplete
                        ...
                        <template v-slot:item="{ item }">
                          <v-list-item-content>
                            <v-list-item-title v-text="item.name" :style="`color: ${item.id.charAt(0) === 'm' ? 'red':'black'}`" />
                          </v-list-item-content>
                        </template>
                      </v-autocomplete>
                    </md-tab>
          ...
          <div v-else>
            ...
            <v-btn 
                  ...
                  class="primary" 
                  @click="createSMFF"
                  :disabled="!$store.state.auth.permissions.companies.includes('U')">Create</v-btn>
          </div>
        </md-card-content>
      </md-card>
      ...
      <v-dialog v-model="standBy" persistent width="300">
        <v-card color="primary" dark>
          <v-card-text>
            Please stand by<v-progress-linear indeterminate color="white" class="mb-0" />
          </v-card-text>
        </v-card>
      </v-dialog>
      <v-dialog v-model="dialogDeleteConfirm" width="500">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn text @click="dialogDeleteConfirm = false" >Cancel</v-btn>
            <v-btn color="primary" text @click="removeSMFF()">Delete SMFF Services</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
  ...
  export default {
    ...
    methods: {
      ...
      updateData (type) {
        ...
        axios.post('companies/' + this.$route.params.id + '/' + type, data)
          .then(res => {
            this.getDataFromApi()
            this.$store.dispatch('notification/message', {
              message: res.data.message
