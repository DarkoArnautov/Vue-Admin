        ...
        <div class="md-layout-item md-xsmall-size-100 md-small-size-100 md-medium-size-100 md-size-40">
          <md-card class="md-card-profile">
            ...
            <md-card-content v-if="editedItem">
              <h6 class="category text-gray">{{ editedItem.email }}</h6>
              <h4 class="card-title">{{editedItem.first_name + ' ' + editedItem.last_name}}</h4>
              <p class="card-description">{{ editedItem.description }}</p>
              <v-btn :color="editedItem.active ? 'primary':'error'" small depressed rounded
                    @click="toggleStatus()" :disabled="!$store.state.auth.permissions.users.includes('U')">
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        itemsMenu: [
          { title: 'Info', icon: 'fas fa-folder-open fa-fw', to: '/home/individuals/view/' + this.$route.params.id },
          {
            title: 'Address',
            icon: 'fas fa-map-marked-alt fa-fw',
            to: '/home/individuals/view/' + this.$route.params.id + '/addresses'
          },
          {
            title: 'SMFF Capabilities',
            icon: 'fas fa-hard-hat fa-fw',
            to: '/home/individuals/view/' + this.$route.params.id + '/smff'
          },
          {
            title: 'Documents',
            icon: 'fas fa-file fa-fw',
            to: '/home/individuals/view/' + this.$route.params.id + '/documents'
          },
          {
            title: 'Notes',
            icon: 'fas fa-sticky-note fa-fw',
            to: '/home/individuals/view/' + this.$route.params.id + '/notes'
          }
        ],
        activeMenu: null,
        loading: true,
    ...
    methods: {
      ...
      deleteCover () {
        this.coverPhoto = this.noImagePhoto
        axios.delete('users/' + this.$route.params.id + '/destroyPhoto')
          .then(res => {
            this.getCover()
      ...
      submitFile ($event) {
        ...
        axios.post('users/' + this.$route.params.id + '/setPhoto',
          formData, {
            headers: {
