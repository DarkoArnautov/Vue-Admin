...
L.Control.Velocity = L.Control.extend({
  ...
  vectorToDegrees: function vectorToDegrees (uMs, vMs) {
    ...
    return velocityDirTrigFromDegrees.toFixed()
  },
  ...
  _onMouseMove: function _onMouseMove (e) {
    ...
    if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
      ...
      htmlOut = '<strong>' + this.options.velocityType + ' Direction: </strong>' + self.vectorToDegrees(gridValue[0], vMs) + '°' + '<br><strong>' + this.options.velocityType + ' Speed: </strong>' + self.vectorToSpeed(gridValue[0], vMs).toFixed(1) + 'm/s | ' + (self.vectorToSpeed(gridValue[0], vMs) * 1.94).toFixed(1) + ' kn'
    } else {
...
L.VelocityLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  onAdd: function onAdd (map) {
    ...
    // layerControl.addOverlay(this._canvasLayer, 'wind');
